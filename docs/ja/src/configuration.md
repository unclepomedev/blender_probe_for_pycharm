...
選択されたマニフェストが意図したものでない場合、またはプロジェクト内の依存関係が独自のマニフェストを含んでいる場合は、**Blender manifest path**（**Settings/Preferences** > **Tools** > **Blender Probe**）に使用したいアドオンのマニフェストを設定してください。空のままにしておくと、自動検索が維持されます。
