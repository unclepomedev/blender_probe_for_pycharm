...
デフォルトでは、Blender Probe はプロジェクト内を検索して `blender_manifest.toml` を見つけることでアドオンを検出します。仮想環境やビルド出力、`tests` などのディレクトリはスキップされます。複数のマニフェストが残っている場合は最も階層の浅いパスが選択され、プラグインはどのマニフェストが選択されたかを通知します。

選択されたマニフェストが意図したものでない場合、またはプロジェクト内の依存関係が独自のマニフェストを含んでいる場合は、**Manifest file override**（**Settings/Preferences** > **Tools** > **Blender Probe**）に使用したいアドオンのマニフェストを設定してください。空のままにしておくと、自動検索が維持されます。
