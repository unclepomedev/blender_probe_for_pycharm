...
class BpyTypesGenerator:
    def __init__(self, context: StubContext, writer: StubWriter):
        ...
        self.tpl_property_readonly = template_loader.get_template("core/property_readonly.pyi")
    ...
    def _generate_single_type(self, name: str, cls: type):
        ...
        self.writer.write_file(self.context.config.bpy_types_dir, f"{name}.pyi", full_content.splitlines())
        ...
        if hasattr(cls, "bl_rna"):
            for prop in cls.bl_rna.properties:
                ...
                if getattr(prop, "is_readonly", False):
                    doc_fmt = self.writer.format_docstring(description, indent="        ") if description else ""
