...
/**
 * Utility functions for handling script resources. Provides methods to extract Python scripts from
 * the plugin resources to the file system.
 */
object ScriptResourceUtils {
    /**
     * Extracts a script from the plugin resources to a temporary directory. The script is expected
     * to be located in the `/python/` directory of the resources.
     *
    ...
    /**
     * Extracts several scripts from the `/python/` resources into a single shared temporary
     * directory, preserving their file names so the entry-point script can import its siblings at
     * runtime (e.g. `probe_server.py` importing `wheels.py`).
     *
     * @param scriptNames The script file names to extract; the first is treated as the entry point
     *   and its extracted file is returned.
    ...
    fun extractResourceScript(resourcePath: String, tempFileNamePrefix: String): File {
        ...
        val stream =
            javaClass.getResourceAsStream(path) ?: throw IOException("Resource not found: $path")
